...
revealed once with a diagonal line-after-line slide-down (CSS keyframes, plays
on load then freezes), a Less->More legend, and a stats footer.
...
def normalize_data(data):
    """Accept both the current JSON schema and the older stats-wrapped schema."""
    days = data.get("days", [])
    if not days:
        raise ValueError("data/contributions.json has no days")

    if "total_contributions" in data:
        return data

    stats = data.get("stats", {})
    total = stats.get("total", sum(d.get("count", 0) for d in days))
    best = stats.get("best_day") or max(days, key=lambda d: d.get("count", 0))

    return {
        "username": data.get("username", "dv2202"),
        "generated_at": data.get("generated_at") or data.get("fetched_at"),
        "range": {"start": days[0]["date"], "end": days[-1]["date"]},
        "total_contributions": total,
        "current_streak": {"length": stats.get("current_streak", 0)},
        "longest_streak": {"length": stats.get("longest_streak", 0)},
        "best_day": {"date": best["date"], "count": best["count"]},
        "days": [{"date": d["date"], "count": d.get("count", 0)} for d in days],
    }
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">devansh@github: ~/contributions --graph</text>')
